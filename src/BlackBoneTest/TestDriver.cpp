...
#define CHECK_AND_SKIP if(_mustSkip) return
    ...
    public:
        ...
        TEST_METHOD( ProtectProcess )
        {
            ...
            Assert::IsTrue( thisProc.core().isProtected() );
        ...
        TEST_METHOD( PromoteHandle )
        {
            ...
            AssertEx::NtSuccess( status );
            Assert::AreEqual( ACCESS_MASK( PROCESS_ALL_ACCESS ), info.GrantedAccess );
        }
        ...
        TEST_METHOD( ReadMemory )
        {
            ...
            Assert::AreEqual( static_cast<short>(IMAGE_DOS_SIGNATURE), *reinterpret_cast<short*>(buf) );
        }
